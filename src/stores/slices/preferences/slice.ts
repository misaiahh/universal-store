...
// Re-export the slice SHAPE so existing `from './slices/preferences/slice'`
// imports (e.g. appStore.ts) keep resolving `PreferencesSlice`; its definition
// lives in ./types to keep interfaces separate from the slice DEFINITION below.
export type { PreferencesSlice } from './types'

// Preferences page slice — DEFINITION only. Its public shape lives in ./types
// and each action lives in its own file under ./actions, so this file is pure
