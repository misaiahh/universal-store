...
import type { BillingSlice } from './types'
...
// Re-export the slice SHAPE so existing `from './slices/billing/slice'` imports
// (e.g. appStore.ts) keep resolving `BillingSlice`; its definition lives in
// ./types to keep interfaces separate from the slice DEFINITION below.
export type { BillingSlice } from './types'

// Billing page slice — DEFINITION only. Its public shape lives in ./types and
