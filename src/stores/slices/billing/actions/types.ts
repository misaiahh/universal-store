...
// calls.
//
// `BillingSet`/`BillingGet` are the exact `set`/`get` types the billing slice
...
// `set`, whole-store `get`).
type BillingStateCreator = StateCreator<
  AppStore,
  [['zustand/immer', never]],
  [],
  unknown
...
export type BillingSet = Parameters<BillingStateCreator>[0]
export type BillingGet = Parameters<BillingStateCreator>[1]
